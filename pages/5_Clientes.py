...
import pandas as pd
from utils import run_query, fmt_brl, fmt_num, CSS, fone_whatsapp, periodo_para_data, sidebar_periodo
...
SQL_CLIENTES = f"""
    ...
    SELECT
        ...
        c.nome_completo,
        c.email,
        CONCAT(COALESCE(c.ddd,''), COALESCE(c.telefone,'')) AS telefone_raw,
        c.ddd,
        c.telefone,
        c.data_nascimento,
        c.cidade,
...
if busca_tel:
    df = df[df["telefone_raw"].str.contains(busca_tel, na=False)]

# ── KPIs ──────────────────────────────────────────────────────────────────────
c1, c2, c3, c4 = st.columns(4)
c1.metric("Clientes encontrados",    fmt_num(len(df)))
c2.metric("Ticket médio",            fmt_brl(df["total_gasto"].mean()))
...
df_exib = df[[
    ...
    "canais", "total_pedidos", "total_gasto",
...
doc_sel = st.selectbox(
    ...
    format_func=lambda d: df[df.documento == d]["nome_completo"].values[0] if not df[df.documento == d].empty else d,
)
